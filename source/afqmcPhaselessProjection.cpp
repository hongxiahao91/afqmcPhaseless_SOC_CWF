...
void AfqmcPhaseless::projectExpMinusHalfDtK()
{
    ...
    for(int i = 0; i < method.walkerSizePerThread; ++i)
    {
        if( walkerIsAlive[i] )
        {
            ...
            walker[i] = move(walkerTemp);
            cout<<"walker i weigh  after half K： "<<walker[i].getLogw()<<endl;
        }
    }
}
...
void AfqmcPhaseless::projectExpMinusDtKExpMinusDtV()
{
    ...
    for(int i = 0; i < method.walkerSizePerThread; ++i)
    {
        if( walkerIsAlive[i] )
        {
            ...
            //cout<<"Everything is correct before two body projection"<<endl;
            //cout<<"twoBodySample.getL(): "<<twoBodySample.getL()<<endl;
            twoBodySampleWalkerRightOperation.applyToRight(twoBodySample, walker[i], walkerTemp);
            cout<<"weight after V: "<<walkerTemp.getLogw()<<endl;
            //cout<<"Everything is correct after two body projection"<<endl;

            walkerTemp.addLogw(method.dt * method.ET);
            cout<<"weight after V+ET: "<<walkerTemp.getLogw()<<endl;

            oneBodyWalkerRightOperation.applyToRight(expMinusDtK, walkerTemp, walker[i]);

            cout<<"weight after one step: "<<walker[i].getLogw()<<endl;
        }
    }
}
