...
void AfqmcPhaseless::initialParameters()
{
    ...
    if( MPIRank()==0 ) model.checkpotential("model_potential");
    if( MPIRank()==0 ) model.checkkinetic("model_kinetic");

    if( MPIRank()==0 ) model.readFCIDUMPSOC();

    if( MPIRank()==0 ) model.checkpotential("FCIDUMP_potential");
    if( MPIRank()==0 ) model.checkkinetic("FCIDUMP_kinetic");

    expMinusDtV = model.returnExpMinusAlphaV( method.dt );
    constForce = expMinusDtV.readForce("constForce_param");

    cout<<"Initiate Parameters Success!"<<endl;
}
...
void AfqmcPhaseless::initialMeasure()
{
    ...
    cout<<"Initiate Measure Success!"<<endl;
}
...
void AfqmcPhaseless::initialExpOneBody()
{
    ...
    {
        addPureMeasure();
        cout<<"Add pure Measure Success!"<<endl;
        adjustETAndBackGroundThenResetMeasurement();
        cout<<"adjustETAndBackGroundThenResetMeasurement Success!"<<endl;
    }
    ...
    cout<<"End of initialExpOneBody Success!"<<endl;
}
...
void AfqmcPhaseless::prepareStop()
{
    ...
    if( MPIRank()==0 )
    {
        const TensorHao<complex<double>, 2> &greenMatrix = walkerWalkerOperation.returnGreenMatrix();
        cout << "check density matrix hermitian:" <<endl;
        for(size_t i = 0; i < 10; ++i)
        {
            cout << greenMatrix(2,i)  << '\t' << greenMatrix(i,2) << endl;
        }
